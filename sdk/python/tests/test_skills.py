...
class SkillsSDKTests(unittest.TestCase):
    def test_catalog_scan_dynamic_review_and_suggestions_routes(self):
        with patch.object(yunque, "_api_call", return_value={"ok": True}) as api_call:
            ...
            yunque.skills_dynamic.list()
            yunque.skills_dynamic.approve("draft_doc", "use safely")
            yunque.skills_dynamic.reject("old_skill")
            yunque.skills.approve("draft_doc", "use safely")
            yunque.skills.reject("old_skill")
            yunque.skills.suggestions("sess-1")
            yunque.skills_suggestions.suggestions("sess-1")
        ...
        self.assertEqual(calls[5], ("GET", "/v1/skills/dynamic"))
        self.assertEqual(calls[6], ("POST", "/v1/skills/approve", {"name": "draft_doc", "instruction": "use safely"}))
        self.assertEqual(calls[7], ("POST", "/v1/skills/reject", {"name": "old_skill"}))
        self.assertEqual(calls[8], ("POST", "/v1/skills/approve", {"name": "draft_doc", "instruction": "use safely"}))
        self.assertEqual(calls[9], ("POST", "/v1/skills/reject", {"name": "old_skill"}))
        self.assertEqual(calls[10], ("GET", "/v1/skill-suggestions?session_id=sess-1"))
        self.assertEqual(calls[11], ("GET", "/v1/skill-suggestions?session_id=sess-1"))
    ...
    def test_agent_kit_exposes_skills_namespace(self):
        ...
        self.assertIs(yunque.create_agent_kit().skills_suggestions, yunque.skills_suggestions)
        self.assertIs(yunque.create_agent_kit().skills_dynamic, yunque.skills_dynamic)
