...
class SkillsSDKTests(unittest.TestCase):
    def test_catalog_scan_dynamic_review_and_suggestions_routes(self):
        ...
        self.assertEqual(calls[1], ("POST", "/v1/skills/scan"))
        self.assertEqual(calls[2], ("GET", "/v1/skills/dynamic"))
        self.assertEqual(calls[3], ("POST", "/v1/skills/approve", {"name": "draft_doc", "instruction": "use safely"}))
        self.assertEqual(calls[4], ("POST", "/v1/skills/reject", {"name": "old_skill"}))
        self.assertEqual(calls[5], ("GET", "/v1/skill-suggestions?session_id=sess-1"))
