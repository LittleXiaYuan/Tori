...
SBOM Drift Pack SDK exposes optional Pack Runtime helpers (`status`, `snapshots`, `createSnapshot`, `snapshot`, `diff`, `evidence`) for external compliance pages, plugin UIs, CLIs, sidecars, and automation scripts without importing the full platform client. It maps directly to `/v1/sbom-drift/status`, `/v1/sbom-drift/snapshots`, `/v1/sbom-drift/diff`, and `/v1/sbom-drift/evidence/*`; install and enable `yunque.pack.sbom-drift` through Pack Runtime before calling the route-gated surface.
