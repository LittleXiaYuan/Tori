...
test("PacksClient reads backend module registry", async () => { const calls: { url: string; init?: RequestInit }[] = []; const client = createPacksClient({ baseUrl: "http://localhost:9090", fetch: async (url, init) => { calls.push({ url: String(url), init }); return jsonResponse({ modules: [{ pack_id: "yunque.pack.backup", routes: [{ path: "/v1/backup/info" }] }], count: 1 }); } }); const modules = await client.backendModules(); assertEqual(modules.count, 1); assertEqual(modules.modules[0]?.pack_id, "yunque.pack.backup"); assertEqual(modules.modules[0]?.routes[0]?.path, "/v1/backup/info"); assertEqual(calls[0]?.url, "http://localhost:9090/v1/packs/backend-modules"); });
