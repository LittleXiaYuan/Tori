...
export class PacksClient {
  ...
  async frontendSync(): Promise<{ menus: PackFrontendMenu[]; routes: PackFrontendRoute[]; sdk: PackSdkEntrypoint[]; packs: InstalledPack[] }> { const response = await this.enabled(); const packs = response.packs ?? []; return { packs, menus: packs.flatMap((pack) => pack.manifest.frontend?.menus ?? []).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)), routes: packs.flatMap((pack) => pack.manifest.frontend?.routes ?? []), sdk: packs.flatMap((pack) => Object.entries(pack.manifest.sdk ?? {}).filter((entry): entry is [string, string] => typeof entry[1] === "string" && entry[1].trim().length > 0).map(([language, importPath]) => ({ packId: pack.manifest.id, packName: pack.manifest.name, language, importPath }))) }; }
