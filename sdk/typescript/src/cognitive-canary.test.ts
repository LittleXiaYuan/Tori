...
test("CognitiveCanaryClient saves scenarios, evaluates canaries, plans shadow traffic, and reads report detail", async () => {
  const calls: { url: string; init?: RequestInit }[] = [];
  const client = createCognitiveCanaryClient({
    baseUrl: "http://localhost:9090",
    apiKey: "key-123",
  ...
  assertEqual(calls[3]?.url, "http://localhost:9090/v1/cognitive-canary/reports/canary-1");
