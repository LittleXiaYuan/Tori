...
const requiredCapabilities = ["get", "update", "skills", "addSkill", "deleteSkill", "presets", "switchPreset", "addCustomPreset", "deleteCustomPreset", "updatePresetFeatures", "modes", "setMode", "currentMode"];
...
for (const route of ["/v1/persona", "/v1/persona/skills", "/v1/persona/presets", "/v1/persona/presets/custom", "/v1/persona/presets/features", "/v1/persona/modes", "/v1/persona/mode", "/v1/persona/mode/current"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Persona SDK|persona identity|persona skills|persona presets|persona modes|mode\/current|v1\/persona/i.test(text)) fail(`${language} doc ${doc} does not mention Persona helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Persona SDK|persona identity|persona skills|persona presets|persona modes|mode\/current|v1\/persona/i.test(text)) fail(`overview doc ${doc} does not describe Persona SDK surface`);
}
