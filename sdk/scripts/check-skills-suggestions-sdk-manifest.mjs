...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/skills-suggestions-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["suggestions"];
...
for (const route of ['"/v1/skill-suggestions"', "handleSkillSuggestions"]) if (!gatewayRoutes.includes(route)) fail(`skills suggestions route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required SkillsSuggestions capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown SkillsSuggestions capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/SkillsSuggestions SDK|session skill suggestions|\/v1\/skill-suggestions/i.test(text)) fail(`${language} doc ${doc} does not mention SkillsSuggestions SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/SkillsSuggestions SDK|session skill suggestions|\/v1\/skill-suggestions/i.test(text)) fail(`overview doc ${doc} does not describe SkillsSuggestions SDK surface`);
}
if (failures.length) { console.error("SkillsSuggestions SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`SkillsSuggestions SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
