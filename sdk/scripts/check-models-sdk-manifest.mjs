...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/models-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "add", "delete"];
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Models capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Models capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Models SDK|Model registry|\/v1\/models/i.test(text)) fail(`${language} doc ${doc} does not mention Models helpers`);
  }
}

for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Models SDK|Model registry|\/v1\/models/i.test(text)) fail(`overview doc ${doc} does not describe Models SDK surface`);
}

if (failures.length) {
  console.error("Models SDK manifest check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`Models SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
