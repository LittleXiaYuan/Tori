...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-reload-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["reload"];
...
for (const route of ['"/v1/plugins/reload"', "handlePluginReload"]) if (!gatewayRoutes.includes(route)) fail(`plugin reload route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginReload capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginReload capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginReload SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`${language} doc ${doc} does not mention PluginReload SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginReload SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`overview doc ${doc} does not describe PluginReload SDK surface`);
}
if (failures.length) { console.error("PluginReload SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginReload SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
