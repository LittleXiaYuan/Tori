...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-folder-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["openFolder"];
...
for (const route of ['"/v1/plugins/open-folder"', "handlePluginOpenFolder"]) if (!gatewayRoutes.includes(route)) fail(`plugin folder route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginFolder capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginFolder capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginFolder SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`${language} doc ${doc} does not mention PluginFolder SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginFolder SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`overview doc ${doc} does not describe PluginFolder SDK surface`);
}
if (failures.length) { console.error("PluginFolder SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginFolder SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
