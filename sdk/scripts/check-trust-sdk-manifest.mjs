...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/trust-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["scores", "reset", "grant", "grantAll", "reviewStatus", "skillGrowPatterns"];
...
for (const route of ["/api/trust/scores", "/api/trust/reset", "/api/trust/grant", "/api/review/status", "/api/skillgrow/patterns"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required trust capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown trust capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Trust SDK|Trust|trust\/scores|review gate|skill-growth/i.test(text)) fail(`${language} doc ${doc} does not mention Trust helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Trust SDK|Trust|trust\/scores|review gate|skill-growth/i.test(text)) fail(`overview doc ${doc} does not describe Trust SDK surface`);
}
if (failures.length) { console.error("Trust SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Trust SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
