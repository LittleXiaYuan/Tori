...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/embeddings-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["providers", "embed"];
...
if (!gatewayRoutes.includes('"/v1/embeddings"') && !gatewayRoutes.includes("/v1/embeddings")) fail("gateway route not found: /v1/embeddings");
if (!gatewayRoutes.includes("handleEmbeddings")) fail("gateway handler not found: handleEmbeddings");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Embeddings capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Embeddings capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Embeddings SDK|embedding provider|\/v1\/embeddings/i.test(text)) fail(`${language} doc ${doc} does not mention Embeddings helpers`);
  }
}

for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Embeddings SDK|embedding provider|\/v1\/embeddings/i.test(text)) fail(`overview doc ${doc} does not describe Embeddings SDK surface`);
}

if (failures.length) {
  console.error("Embeddings SDK manifest check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`Embeddings SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
