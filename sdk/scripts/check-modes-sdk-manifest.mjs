...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/modes-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "current", "set"];
...
for (const route of ["/v1/persona/modes", "/v1/persona/mode", "/v1/persona/mode/current"]) if (!gatewayRoutes.includes(`"${route}"`) && !gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
for (const handler of ["handleListModes", "handleSetMode", "handleCurrentMode"]) if (!gatewayRoutes.includes(handler)) fail(`gateway handler not found: ${handler}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Modes capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Modes capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Modes SDK|persona modes|\/v1\/persona\/mode/i.test(text)) fail(`${language} doc ${doc} does not mention Modes helpers`);
  }
}

for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Modes SDK|persona modes|\/v1\/persona\/mode/i.test(text)) fail(`overview doc ${doc} does not describe Modes SDK surface`);
}

if (failures.length) {
  console.error("Modes SDK manifest check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`Modes SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
