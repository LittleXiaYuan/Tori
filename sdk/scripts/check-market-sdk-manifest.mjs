...
const manifestPath = resolve(repoRoot, "sdk/manifest/market-sdk.json");
...
const requiredCapabilities = ["search", "top", "stats"];
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required market capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown market capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/market|Market|marketplace|Skill Market|技能市场/.test(text)) fail(`${language} doc ${doc} does not mention market helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/market|Market|marketplace|Skill Market|技能市场/.test(text)) fail(`overview doc ${doc} does not describe Market SDK surface`);
}
if (failures.length) { console.error("Skill Market SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Skill Market SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
