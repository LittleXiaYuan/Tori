...
for (const handler of ["handlePacksList", "handlePacksEnabled", "handlePackBackendModules", "handlePackInstall", "handlePackEnable", "handlePackDisable", "handlePackRollback", "RegisterBackendPack", "registerBackendPack", "requirePackRoute", "backendPackRouteInfos", "BackendRouteInfo{Method", "route.Method = strings.ToUpper", "must declare an HTTP method"]) if (!gateway.includes(handler)) fail(`pack runtime gateway contract not found: ${handler}`);
...
for (const token of ["Manifest", "Registry", "BackendModule", "BackendRoute", "Method  string", "BackendRouteSpec", "routeSpecs", "AllowsRoute", "BackendModuleInfo", "BackendRouteInfo", "method,omitempty", "frontend", "menus", "routes", "sdk", "DistributionManifest", "PackArtifacts", "PreviousArtifacts", "PruneArtifacts", "CacheDistribution", "InstallWithArtifacts", "packageUrl", "frontendUrl", "sha256", "rollback"]) if (!packContract.includes(token)) fail(`pack runtime contract missing token: ${token}`);
