...
for (const token of ["Manifest", "Registry", "BackendModule", "BackendRoute", "Method  string", "BackendModuleInfo", "BackendRouteInfo", "method,omitempty", "frontend", "menus", "routes", "sdk", "DistributionManifest", "packageUrl", "frontendUrl", "sha256", "rollback"]) if (!packContract.includes(token)) fail(`pack runtime contract missing token: ${token}`);
...
for (const token of ["PackSdkEntrypoint", "PackDistributionManifest", "sdk: packs.flatMap", "Object.entries(pack.manifest.sdk", "importPath", "distributions:", "pack.manifest.distribution"]) if (!packsSource.includes(token)) fail(`Packs SDK frontendSync missing SDK/distribution sync token: ${token}`);
