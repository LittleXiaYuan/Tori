...
for (const handler of ["handlePacksList", "handlePacksEnabled", "handlePackBackendModules", "handlePackInstall", "handlePackEnable", "handlePackDisable", "handlePackRollback", "RegisterBackendPack", "registerBackendPack", "requirePackRoute", "backendPackRouteInfos", "BackendRouteInfo{Method", "route.Method = strings.TrimSpace"]) if (!gateway.includes(handler)) fail(`pack runtime gateway contract not found: ${handler}`);
...
for (const token of ["Manifest", "Registry", "BackendModule", "BackendRoute", "Method  string", "BackendModuleInfo", "BackendRouteInfo", "method,omitempty", "frontend", "menus", "routes", "sdk", "DistributionManifest", "PackArtifacts", "CacheDistribution", "InstallWithArtifacts", "packageUrl", "frontendUrl", "sha256", "rollback"]) if (!packContract.includes(token)) fail(`pack runtime contract missing token: ${token}`);
...
for (const token of ["PackSdkEntrypoint", "PackDistributionManifest", "PackArtifacts", "download?: boolean", "sdk: packs.flatMap", "Object.entries(pack.manifest.sdk", "importPath", "distributions:", "pack.manifest.distribution"]) if (!packsSource.includes(token)) fail(`Packs SDK frontendSync missing SDK/distribution sync token: ${token}`);
