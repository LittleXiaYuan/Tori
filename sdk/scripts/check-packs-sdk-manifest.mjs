...
const requiredCapabilities = ["list", "installed", "enabled", "install", "enable", "disable", "rollback", "frontendSync"];
const requiredRoutes = ["GET /v1/packs", "GET /v1/packs/installed", "GET /v1/packs/enabled", "POST /v1/packs/install", "POST /v1/packs/enable", "POST /v1/packs/disable", "POST /v1/packs/rollback"];
...
for (const route of ['"/v1/packs"', '"/v1/packs/installed"', '"/v1/packs/enabled"', '"/v1/packs/install"', '"/v1/packs/enable"', '"/v1/packs/disable"', '"/v1/packs/rollback"']) if (!gateway.includes(route)) fail(`gateway route not found: ${route}`);
for (const handler of ["handlePacksList", "handlePacksEnabled", "handlePackInstall", "handlePackEnable", "handlePackDisable", "handlePackRollback", "RegisterBackendPack", "registerBackendPack", "requirePackRoute"]) if (!gateway.includes(handler)) fail(`pack runtime gateway contract not found: ${handler}`);
...
for (const token of ["Manifest", "Registry", "BackendModule", "BackendRoute", "frontend", "menus", "routes", "sdk", "rollback"]) if (!packContract.includes(token)) fail(`pack runtime contract missing token: ${token}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Pack Runtime SDK|Packs SDK|createPacksClient|frontendSync|v1\/packs/i.test(text)) fail(`${language} doc ${doc} does not mention Packs SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Pack Runtime SDK|Packs SDK|frontendSync|v1\/packs/i.test(text)) fail(`overview doc ${doc} does not describe Packs SDK surface`);
}
