...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-llm-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["complete"];
...
for (const route of ['"/v1/plugin-api/llm"', "handleLLM"]) if (!gatewayRoutes.includes(route)) fail(`plugin llm route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginLLM capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginLLM capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginLLM SDK|plugin-scoped LLM|\/v1\/plugin-api\/llm/i.test(text)) fail(`${language} doc ${doc} does not mention PluginLLM SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginLLM SDK|plugin-scoped LLM|\/v1\/plugin-api\/llm/i.test(text)) fail(`overview doc ${doc} does not describe PluginLLM SDK surface`);
}
if (failures.length) { console.error("PluginLLM SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginLLM SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
