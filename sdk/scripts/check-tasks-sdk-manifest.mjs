...
const requiredCapabilities = ["list", "get", "create", "run", "pause", "resume", "restart", "cancel", "delete", "templates", "template", "createTemplate", "deleteTemplate", "instantiateTemplate", "gaps", "gapStats", "resolveGap", "workingMemory"];
...
for (const route of ['"/v1/tasks"', '"/v1/tasks/run"', '"/v1/tasks/pause"', '"/v1/tasks/resume"', '"/v1/tasks/restart"', '"/v1/tasks/cancel"', '"/v1/tasks/templates"', '"/v1/tasks/templates/instantiate"', '"/v1/tasks/gaps"', '"/v1/tasks/gaps/resolve"', '"/v1/tasks/memory"']) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
for (const handler of ["handleTaskList", "handleTaskCreate", "handleTaskRun", "handleTaskPause", "handleTaskResume", "handleTaskRestart", "handleTaskCancel", "handleTaskDelete", "handleTemplates", "handleTemplateInstantiate", "handleGaps", "handleGapResolve", "handleTaskWorkingMemory"]) if (!gatewayRoutes.includes(handler)) fail(`gateway handler not found: ${handler}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Tasks SDK|task CRUD|task templates|task gaps|working memory|task pages|v1\/tasks/i.test(text)) fail(`${language} doc ${doc} does not mention Tasks helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Tasks SDK|task CRUD|task templates|task gaps|working memory|task pages|v1\/tasks/i.test(text)) fail(`overview doc ${doc} does not describe Tasks SDK surface`);
}
