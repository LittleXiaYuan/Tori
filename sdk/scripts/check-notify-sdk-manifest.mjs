...
const manifestPath = resolve(repoRoot, "sdk/manifest/notify-sdk.json");
...
const requiredCapabilities = ["channels", "addChannel", "removeChannel", "toggleChannel", "testChannel", "share"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/notifyapi/handler.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required notify capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown notify capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/notify|Notify|notification|通知/.test(text)) fail(`${language} doc ${doc} does not mention notify helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/notify|Notify|notification|通知/.test(text)) fail(`overview doc ${doc} does not describe Notify SDK surface`);
}
if (failures.length) { console.error("Notify SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Notify SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
