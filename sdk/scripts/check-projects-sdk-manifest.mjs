...
const manifestPath = resolve(repoRoot, "sdk/manifest/projects-sdk.json");
...
const requiredCapabilities = ["list", "create", "detail", "update", "remove"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/handlers_projects.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required projects capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown projects capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/projects|Projects|project workspace|项目/.test(text)) fail(`${language} doc ${doc} does not mention projects helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/projects|Projects|project workspace|项目/.test(text)) fail(`overview doc ${doc} does not describe Projects SDK surface`);
}
if (failures.length) { console.error("Projects SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Projects SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
