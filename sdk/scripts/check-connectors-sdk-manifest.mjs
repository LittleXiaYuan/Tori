...
const manifestPath = resolve(repoRoot, "sdk/manifest/connectors-sdk.json");
...
const requiredCapabilities = ["list", "detail", "connect", "disconnect", "execute"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/connectorapi/handler.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required connector capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown connector capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/connector|Connector|连接器/.test(text)) fail(`${language} doc ${doc} does not mention connector helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/connector|Connector|连接器/.test(text)) fail(`overview doc ${doc} does not describe Connector SDK surface`);
}
if (failures.length) { console.error("Connectors SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Connectors SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
