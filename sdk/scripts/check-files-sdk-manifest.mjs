...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/files-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "preview", "download"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_files.go");
for (const route of ['"/api/files"', '"/api/files/preview"', '"/api/files/download"']) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Files capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Files capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Files SDK|artifact|\/api\/files/.test(text)) fail(`${language} doc ${doc} does not mention Files helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Files SDK|artifact|\/api\/files/.test(text)) fail(`overview doc ${doc} does not describe Files SDK surface`);
}
if (failures.length) { console.error("Files SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Files SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
