...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/emotion-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["history", "stickers", "registerStickers", "clearStickers"];
...
for (const route of ["/v1/emotion/history", "/v1/emotion/stickers"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required emotion capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown emotion capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Emotion SDK|emotion history|emotion stickers|v1\/emotion/i.test(text)) fail(`${language} doc ${doc} does not mention Emotion helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Emotion SDK|emotion history|emotion stickers|v1\/emotion/i.test(text)) fail(`overview doc ${doc} does not describe Emotion SDK surface`);
}
if (failures.length) { console.error("Emotion SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Emotion SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
