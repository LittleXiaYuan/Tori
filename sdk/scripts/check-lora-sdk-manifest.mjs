...
const manifestPath = resolve(repoRoot, "sdk/manifest/lora-sdk.json");
...
const requiredCapabilities = ["status", "history", "summary", "preview", "trigger", "rollback", "evolution", "config", "updateConfig"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/loraapi/handler.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required LoRA capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/lora|LoRA|adapter|训练|进化/.test(text)) fail(`${language} doc ${doc} does not mention LoRA helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/lora|LoRA|adapter|训练|进化/.test(text)) fail(`overview doc ${doc} does not describe LoRA SDK surface`);
}
if (failures.length) { console.error("LoRA SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`LoRA SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
