...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/upload-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["file", "upload"];
...
for (const route of ['"/v1/upload"', "handleFileUpload"]) if (!gatewayRoutes.includes(route)) fail(`upload route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Upload capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Upload capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Upload SDK|artifact upload|parsed-file|\/v1\/upload/i.test(text)) fail(`${language} doc ${doc} does not mention Upload SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Upload SDK|artifact upload|parsed-file|\/v1\/upload/i.test(text)) fail(`overview doc ${doc} does not describe Upload SDK surface`);
}
if (failures.length) { console.error("Upload SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Upload SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
