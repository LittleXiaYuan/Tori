...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/instructions-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "create", "update", "delete", "reorder"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_instructions.go");
for (const route of ["/v1/instructions", "/v1/instructions/reorder"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required instructions capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown instructions capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Instructions SDK|user instructions|instruction CRUD|v1\/instructions/i.test(text)) fail(`${language} doc ${doc} does not mention Instructions helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Instructions SDK|user instructions|instruction CRUD|v1\/instructions/i.test(text)) fail(`overview doc ${doc} does not describe Instructions SDK surface`);
}
if (failures.length) { console.error("Instructions SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Instructions SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
