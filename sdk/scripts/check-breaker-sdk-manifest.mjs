...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/breaker-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["reset"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_providers.go");
for (const route of ["/api/breaker/reset", "handleBreakerReset"]) if (!gatewayRoutes.includes(route)) fail(`breaker route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Breaker capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Breaker capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Breaker|circuit breaker|\/api\/breaker\/reset/i.test(text)) fail(`${language} doc ${doc} does not mention Breaker helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Breaker|circuit breaker|\/api\/breaker\/reset/i.test(text)) fail(`overview doc ${doc} does not describe Breaker SDK surface`);
}
if (failures.length) { console.error("Breaker SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Breaker SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
