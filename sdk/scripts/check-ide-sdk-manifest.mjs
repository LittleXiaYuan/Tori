...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/ide-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["status", "review", "reviewDiff", "reviewQuick", "reviewFull"];
...
const routes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_ide.go");
for (const route of ['"/v1/ide/status"', '"/v1/ide/review"']) if (!routes.includes(route)) fail(`gateway route not found: ${route}`);
for (const handler of ["handleIDEStatus", "handleIDEReviewCode"]) if (!routes.includes(handler)) fail(`gateway handler not found: ${handler}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required IDE capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown IDE capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/IDE SDK|IDE supervisor|code review|v1\/ide/i.test(text)) fail(`${language} doc ${doc} does not mention IDE SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/IDE SDK|IDE supervisor|code review|v1\/ide/i.test(text)) fail(`overview doc ${doc} does not describe IDE SDK surface`);
}
if (failures.length) { console.error("IDE SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`IDE SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
