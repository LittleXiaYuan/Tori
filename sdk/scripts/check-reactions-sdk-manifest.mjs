...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/reactions-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["react", "sendSticker"];
...
for (const route of ["/v1/react", "/v1/sticker/send"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required reactions capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown reactions capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Reactions SDK|emoji reactions|sticker sending|v1\/react|v1\/sticker\/send/i.test(text)) fail(`${language} doc ${doc} does not mention Reactions helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Reactions SDK|emoji reactions|sticker sending|v1\/react|v1\/sticker\/send/i.test(text)) fail(`overview doc ${doc} does not describe Reactions SDK surface`);
}
if (failures.length) { console.error("Reactions SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Reactions SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
