...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/skills-scan-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["scan"];
...
for (const route of ['"/v1/skills/scan"', "handleSkillsScan"]) if (!gatewayRoutes.includes(route)) fail(`skills scan route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required SkillsScan capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown SkillsScan capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/SkillsScan SDK|runtime skill scanning|\/v1\/skills\/scan/i.test(text)) fail(`${language} doc ${doc} does not mention SkillsScan SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/SkillsScan SDK|runtime skill scanning|\/v1\/skills\/scan/i.test(text)) fail(`overview doc ${doc} does not describe SkillsScan SDK surface`);
}
if (failures.length) { console.error("SkillsScan SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`SkillsScan SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
