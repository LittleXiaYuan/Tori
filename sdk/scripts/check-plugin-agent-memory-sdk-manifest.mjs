...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-agent-memory-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["search", "add"];
...
for (const route of ['"/v1/plugin-api/agent-memory/search"', 'handleAgentMemSearch', '"/v1/plugin-api/agent-memory/add"', 'handleAgentMemAdd']) if (!gatewayRoutes.includes(route)) fail(`plugin agent memory route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginAgentMemory capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginAgentMemory capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginAgentMemory SDK|shared Agent memory|\/v1\/plugin-api\/agent-memory\/search/i.test(text)) fail(`${language} doc ${doc} does not mention PluginAgentMemory SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginAgentMemory SDK|shared Agent memory|\/v1\/plugin-api\/agent-memory\/search/i.test(text)) fail(`overview doc ${doc} does not describe PluginAgentMemory SDK surface`);
}
if (failures.length) { console.error("PluginAgentMemory SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginAgentMemory SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
