...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-cron-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["add", "remove", "list"];
...
for (const route of ['"/v1/plugin-api/cron/add"', 'handleCronAdd', '"/v1/plugin-api/cron/remove"', 'handleCronRemove', '"/v1/plugin-api/cron/list"', 'handleCronList']) if (!gatewayRoutes.includes(route)) fail(`plugin cron route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginCron capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginCron capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginCron SDK|plugin-scoped cron|\/v1\/plugin-api\/cron\/add/i.test(text)) fail(`${language} doc ${doc} does not mention PluginCron SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginCron SDK|plugin-scoped cron|\/v1\/plugin-api\/cron\/add/i.test(text)) fail(`overview doc ${doc} does not describe PluginCron SDK surface`);
}
if (failures.length) { console.error("PluginCron SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginCron SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
