...
const manifestPath = resolve(repoRoot, "sdk/manifest/reflect-sdk.json");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/reflect|Reflect|reflection|Experience|经验/.test(text)) {
      fail(`${language} doc ${doc} does not mention reflection experience helpers`);
    }
  }
}
...
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Reflect|Reflection|Experience|经验复用/.test(text)) {
    fail(`overview doc ${doc} does not describe reflection SDK surface`);
  }
}

if (failures.length) {
  console.error("reflect SDK manifest check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`reflect SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
