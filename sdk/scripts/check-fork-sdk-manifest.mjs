...
const manifestPath = resolve(repoRoot, "sdk/manifest/fork-sdk.json");
...
const requiredCapabilities = ["root", "get", "create", "remove", "branch", "list"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/forkapi/handler.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required fork capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown fork capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/fork|Fork|branch|Branch|conversation|Conversation/.test(text)) fail(`${language} doc ${doc} does not mention fork helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/fork|Fork|branch|Branch|conversation|Conversation/.test(text)) fail(`overview doc ${doc} does not describe Fork SDK surface`);
}
if (failures.length) { console.error("Fork SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Fork SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
