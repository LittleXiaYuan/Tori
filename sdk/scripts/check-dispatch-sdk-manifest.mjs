...
const manifestPath = resolve(repoRoot, "sdk/manifest/dispatch-sdk.json");
...
const requiredCapabilities = ["workers", "worker", "removeWorker", "queue", "enqueue", "workerConfig"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/handlers_mcp_dispatch.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required dispatch capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown dispatch capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/dispatch|Dispatch|worker|Worker|MCP/.test(text)) fail(`${language} doc ${doc} does not mention dispatch helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/dispatch|Dispatch|worker|Worker|MCP/.test(text)) fail(`overview doc ${doc} does not describe Dispatch SDK surface`);
}
if (failures.length) { console.error("Dispatch SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Dispatch SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
