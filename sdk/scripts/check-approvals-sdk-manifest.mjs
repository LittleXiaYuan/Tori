...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/approvals-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "pending", "history", "approve", "deny", "decide", "rules", "addRule", "deleteRule"];
...
for (const route of ['"/v1/approvals"', '"/v1/approvals/approve"', '"/v1/approvals/deny"', '"/v1/approvals/decide"', '"/v1/approvals/rules"']) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required approvals capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown approvals capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Approvals SDK|approval|\/v1\/approvals/.test(text)) fail(`${language} doc ${doc} does not mention Approvals helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Approvals SDK|approval|\/v1\/approvals/.test(text)) fail(`overview doc ${doc} does not describe Approvals SDK surface`);
}
if (failures.length) { console.error("Approvals SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Approvals SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
