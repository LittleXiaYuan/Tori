...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/iterate-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["proposals", "pendingProposals", "approve", "reject", "trigger", "status"];
...
for (const route of ["/api/iterate/proposals", "/api/iterate/approve", "/api/iterate/reject", "/api/iterate/trigger", "/api/iterate/status"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required iterate capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown iterate capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Iterate SDK|self-iteration|iterate\/proposals|proposal review|manual cycle/i.test(text)) fail(`${language} doc ${doc} does not mention Iterate helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Iterate SDK|self-iteration|iterate\/proposals|proposal review|manual cycle/i.test(text)) fail(`overview doc ${doc} does not describe Iterate SDK surface`);
}
if (failures.length) { console.error("Iterate SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Iterate SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
