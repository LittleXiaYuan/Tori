...
const manifestPath = resolve(repoRoot, "sdk/manifest/orchestrator-sdk.json");
...
const requiredCapabilities = ["status", "toggle", "sessions", "detectIDEs", "events", "taskTimeline", "policy", "updatePolicy", "addAdapter"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/handlers_orchestrator.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required orchestrator capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown orchestrator capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/orchestrator|Orchestrator|daemon|adapter|IDE|session|policy/.test(text)) fail(`${language} doc ${doc} does not mention orchestrator helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/orchestrator|Orchestrator|daemon|adapter|IDE|session|policy/.test(text)) fail(`overview doc ${doc} does not describe Orchestrator SDK surface`);
}
if (failures.length) { console.error("Orchestrator SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Orchestrator SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
