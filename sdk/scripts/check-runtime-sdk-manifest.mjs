...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/runtime-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["queues", "sessionQueue", "cancelQueuedTask", "events"];
...
for (const route of ["/v1/sessions/queue", "/v1/sessions/queue/cancel", "/v1/events/stream"]) {
  if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
}
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required runtime capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown runtime capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Runtime SDK|runtime|sessions\/queue|events\/stream/i.test(text)) fail(`${language} doc ${doc} does not mention Runtime helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Runtime SDK|runtime|sessions\/queue|events\/stream/i.test(text)) fail(`overview doc ${doc} does not describe Runtime SDK surface`);
}
if (failures.length) { console.error("Runtime SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Runtime SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
