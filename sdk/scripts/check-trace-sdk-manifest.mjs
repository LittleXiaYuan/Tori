...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/trace-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["recent", "byTraceId", "byTaskId"];
...
for (const route of manifest.routes ?? []) {
  ...
  const candidates = [rawPath, rawPath.replace("/{trace_id}", "/"), rawPath.replace("/{task_id}", "/")];
  if (!candidates.some((path) => gatewayRoutes.includes(path))) fail(`gateway route not found for manifest route: ${route}`);
}
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required trace capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown trace capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Trace|trace|audit|replay/.test(text)) fail(`${language} doc ${doc} does not mention Trace helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Trace SDK|trace|audit|replay/.test(text)) fail(`overview doc ${doc} does not describe Trace SDK surface`);
}
if (failures.length) { console.error("Trace SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Trace SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
