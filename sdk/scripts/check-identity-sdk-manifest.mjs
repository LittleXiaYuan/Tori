...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/identity-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["resolve", "profiles"];
...
for (const route of ["/v1/identity/resolve", "/v1/identity/profiles"]) {
  if (!gatewayRoutes.includes(`"${route}"`) && !gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
}
for (const handler of ["handleIdentityResolve", "handleIdentityProfiles"]) {
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Identity capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Identity capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Identity SDK|identity resolution|\/v1\/identity/i.test(text)) fail(`${language} doc ${doc} does not mention Identity helpers`);
  }
}

for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Identity SDK|identity resolution|\/v1\/identity/i.test(text)) fail(`overview doc ${doc} does not describe Identity SDK surface`);
}

if (failures.length) {
  console.error("Identity SDK manifest check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`Identity SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
