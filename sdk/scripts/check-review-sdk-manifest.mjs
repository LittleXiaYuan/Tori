...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/review-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["status"];
...
for (const route of ['"/api/review/status"', "handleReviewStatus"]) if (!gatewayRoutes.includes(route)) fail(`review route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Review capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Review capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Review SDK|review-gate|review gate|\/api\/review\/status/i.test(text)) fail(`${language} doc ${doc} does not mention Review SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Review SDK|review-gate|review gate|\/api\/review\/status/i.test(text)) fail(`overview doc ${doc} does not describe Review SDK surface`);
}
if (failures.length) { console.error("Review SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Review SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
