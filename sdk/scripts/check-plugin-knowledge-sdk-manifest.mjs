...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-knowledge-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["search", "ingest"];
...
for (const route of ['"/v1/plugin-api/knowledge/search"', 'handleKnowledgeSearch', '"/v1/plugin-api/knowledge/ingest"', 'handleKnowledgeIngest']) if (!gatewayRoutes.includes(route)) fail(`plugin knowledge route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginKnowledge capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginKnowledge capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginKnowledge SDK|plugin-scoped knowledge|\/v1\/plugin-api\/knowledge\/search/i.test(text)) fail(`${language} doc ${doc} does not mention PluginKnowledge SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginKnowledge SDK|plugin-scoped knowledge|\/v1\/plugin-api\/knowledge\/search/i.test(text)) fail(`overview doc ${doc} does not describe PluginKnowledge SDK surface`);
}
if (failures.length) { console.error("PluginKnowledge SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginKnowledge SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
