...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-ui-sdk.json"), "utf8"));
const failures = [];
...
for (const route of ['"/v1/plugins/ui"', "handlePluginUI"]) if (!gatewayRoutes.includes(route)) fail(`plugin UI route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginUI capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginUI capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginUI SDK|plugin UI|UI tabs|\/v1\/plugins\/ui/i.test(text)) fail(`${language} doc ${doc} does not mention PluginUI SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginUI SDK|plugin UI|UI tabs|\/v1\/plugins\/ui/i.test(text)) fail(`overview doc ${doc} does not describe PluginUI SDK surface`);
}
if (failures.length) { console.error("PluginUI SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginUI SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
