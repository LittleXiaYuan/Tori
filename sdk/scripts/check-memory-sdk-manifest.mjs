...
const manifestPath = resolve(repoRoot, "sdk/manifest/memory-sdk.json");
...
const requiredCapabilities = ["stats", "search", "add", "compact"];
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required memory capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/memory|Memory|记忆|回忆/.test(text)) fail(`${language} doc ${doc} does not mention memory helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/memory|Memory|记忆|回忆/.test(text)) fail(`overview doc ${doc} does not describe memory SDK surface`);
}
if (failures.length) { console.error("memory SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`memory SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
