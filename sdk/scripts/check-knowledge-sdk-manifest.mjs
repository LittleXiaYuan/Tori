...
const manifestPath = resolve(repoRoot, "sdk/manifest/knowledge-sdk.json");
...
const requiredCapabilities = ["search", "sources", "stats", "ingest", "importUrl", "importRepo", "updateSource", "deleteSource"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/handlers_knowledge.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_knowledge_upload.go") + "\n" + readRepoFile("internal/controlplane/gateway/routes.go");
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required knowledge capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/knowledge|Knowledge|知识库|RAG/.test(text)) fail(`${language} doc ${doc} does not mention knowledge helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/knowledge|Knowledge|知识库|RAG/.test(text)) fail(`overview doc ${doc} does not describe knowledge SDK surface`);
}
if (failures.length) { console.error("knowledge SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`knowledge SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
