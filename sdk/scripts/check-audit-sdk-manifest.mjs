...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/audit-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["tail", "verify", "stats", "trail"];
...
for (const route of ["/v1/audit/tail", "/v1/audit/verify", "/v1/audit/stats", "/api/audit/trail"]) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required audit capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown audit capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Audit SDK|Audit|audit\/tail|audit-chain/i.test(text)) fail(`${language} doc ${doc} does not mention Audit helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Audit SDK|Audit|audit\/tail|audit-chain/i.test(text)) fail(`overview doc ${doc} does not describe Audit SDK surface`);
}
if (failures.length) { console.error("Audit SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Audit SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
