...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/tori-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["bind", "status", "unbind", "health", "usage"];
...
const routes = readRepoFile("internal/controlplane/gateway/routes_system.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_tori.go");
for (const route of ['"/v1/tori/bind"', '"/v1/tori/status"', '"/v1/tori/unbind"', '"/v1/tori/health"', '"/v1/tori/usage"']) if (!routes.includes(route)) fail(`gateway route not found: ${route}`);
for (const handler of ["handleToriBind", "handleToriStatus", "handleToriUnbind", "handleToriHealth", "handleToriUsage"]) if (!routes.includes(handler)) fail(`gateway handler not found: ${handler}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Tori capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Tori SDK|Tori account|Tori binding|v1\/tori/i.test(text)) fail(`${language} doc ${doc} does not mention Backup SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Tori SDK|Tori account|Tori binding|v1\/tori/i.test(text)) fail(`overview doc ${doc} does not describe Backup SDK surface`);
}
if (failures.length) { console.error("Tori SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Tori SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
