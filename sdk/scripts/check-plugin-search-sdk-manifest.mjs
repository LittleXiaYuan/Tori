...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/plugin-search-sdk.json"), "utf8"));
const failures = [];
...
for (const route of ['"/v1/plugin-api/search"', "handleSearch"]) if (!gatewayRoutes.includes(route)) fail(`plugin search route/handler not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required PluginSearch capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown PluginSearch capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/PluginSearch SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`${language} doc ${doc} does not mention PluginSearch SDK helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/PluginSearch SDK|plugin enable\/disable|\/v1\/plugins\/toggle/i.test(text)) fail(`overview doc ${doc} does not describe PluginSearch SDK surface`);
}
if (failures.length) { console.error("PluginSearch SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`PluginSearch SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
