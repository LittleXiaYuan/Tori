...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/chat-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["send", "stream", "agentic", "parseStream"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_chat.go") + "\n" + readRepoFile("internal/controlplane/gateway/websocket.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_agentic.go");
for (const route of ['"/v1/chat"', '"/v1/chat/stream"', '"/v1/chat/agentic"']) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required chat capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown chat capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Chat SDK|chat|\/v1\/chat/.test(text)) fail(`${language} doc ${doc} does not mention Chat helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Chat SDK|chat|\/v1\/chat/.test(text)) fail(`overview doc ${doc} does not describe Chat SDK surface`);
}
if (failures.length) { console.error("Chat SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Chat SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
