...
const manifest = JSON.parse(readFileSync(resolve(repoRoot, "sdk/manifest/bots-sdk.json"), "utf8"));
const failures = [];
...
const requiredCapabilities = ["list", "create", "get", "update", "setActive", "delete", "inbox", "pushInbox", "deleteInbox", "markInboxRead", "markAllInboxRead", "channelGroups"];
...
const gatewayRoutes = readRepoFile("internal/controlplane/gateway/routes.go") + "\n" + readRepoFile("internal/controlplane/gateway/handlers_botops.go");
for (const route of ['"/v1/bots"', '"/v1/bots/detail"', '"/v1/inbox"', '"/v1/inbox/read"', '"/v1/channels/groups"']) if (!gatewayRoutes.includes(route)) fail(`gateway route not found: ${route}`);
...
for (const [language, config] of Object.entries(manifest.languages ?? {})) {
  ...
  for (const capability of requiredCapabilities) if (!config.entrypoints?.[capability]) fail(`${language} entrypoints missing required Bots capability: ${capability}`);
  for (const [capability, symbol] of Object.entries(config.entrypoints ?? {})) {
    if (!capabilityNames.has(capability)) fail(`${language} entrypoint references unknown Bots capability: ${capability}`);
  ...
  for (const doc of config.docs ?? []) {
    ...
    if (!/Bots SDK|bot|inbox|channel group|\/v1\/bots/i.test(text)) fail(`${language} doc ${doc} does not mention Bots helpers`);
  }
}
for (const doc of manifest.overviewDocs ?? []) {
  const text = readRepoFile(doc);
  if (!/Bots SDK|bot|inbox|channel group|\/v1\/bots/i.test(text)) fail(`overview doc ${doc} does not describe Bots SDK surface`);
}
if (failures.length) { console.error("Bots SDK manifest check failed:"); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
console.log(`Bots SDK manifest ok: ${Object.keys(manifest.languages ?? {}).length} languages, ${capabilityNames.size} capabilities`);
