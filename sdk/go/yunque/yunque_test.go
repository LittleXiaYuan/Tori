...
func TestTrustHelpers(t *testing.T) {
	...
	if len(seen) != 12 || seen[0] != "GET /api/trust/scores" || seen[4] != "GET /api/skillgrow/patterns" || seen[5] != "GET /api/skillgrow/patterns" || seen[6] != "GET /api/review/status" {
...
func TestRBACHelpers(t *testing.T) {
	...
	if len(seen) != 12 || seen[0] != "GET /v1/rbac/roles" || seen[2] != "DELETE /v1/rbac/roles?id=operator" || seen[6] != "GET /v1/rbac/my-roles" {
...
func TestApprovalsHelpers(t *testing.T) {
	...
	if len(seen) != 12 || seen[0] != "GET /v1/approvals?status=pending" || seen[1] != "GET /v1/approvals?history=true&status=approved" || seen[7] != "DELETE /v1/approvals/rules?id=r1" {
...
func TestAgentKitGroupsStateReflectAndPluginRuntime(t *testing.T) {
	...
	withTestAPI(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/plugin-api/send":
			_, _ = w.Write([]byte(`{"ok":true}`))
		case "/v1/plugin-api/llm":
			_, _ = w.Write([]byte(`{"reply":"plugin llm reply"}`))
		case "/v1/plugin-api/memory/set":
			_, _ = w.Write([]byte(`{"ok":true}`))
		case "/v1/plugin-api/memory/get":
			_, _ = w.Write([]byte(`{"value":"memory-value"}`))
		case "/v1/plugin-api/knowledge/search":
			_, _ = w.Write([]byte(`{"results":[{"id":"chunk-1"}]}`))
		case "/v1/plugin-api/agent-memory/search":
			_, _ = w.Write([]byte(`{"context":"agent memory context"}`))
		default:
			t.Fatalf("unexpected request: %s %s", r.Method, r.URL.String())
		}
	})
	...
	standaloneResults, err := kit.PluginSearch.Search(context.Background(), "agent kit", 2)
	if err != nil {
		t.Fatal(err)
	}
	if err := kit.PluginSend.Send(context.Background(), "webhook", "ops", "hello"); err != nil {
		t.Fatal(err)
	}
	pluginReply, err := kit.PluginLLM.Chat(context.Background(), []Message{{Role: "user", Content: "hi"}}, 0.2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPluginsNamespaceManagesPluginLifecycle(t *testing.T) {
	...
	filesStandalone, _ := PluginFiles.Files(ctx, "demo")
	saved, _ := Plugins.SaveFile(ctx, "demo", "handler.py", "print('ok')", "demo")
	savedStandalone, _ := PluginFiles.SaveFile(ctx, "demo", "handler.py", "print('ok')", "demo")
	ui, _ := Plugins.UI(ctx)
	...
	openedStandalone, _ := PluginFolder.OpenFolder(ctx, "demo")

	if list["plugins"] == nil || toggled["enabled"] != true || toggledStandalone["enabled"] != true || created["name"] != "demo" || deleted["status"] != "deleted" || files["files"] == nil || filesStandalone["files"] == nil || saved["status"] != "saved" || savedStandalone["status"] != "saved" || ui["tabs"] == nil || pluginUI["tabs"] == nil || reloaded["skills"].(float64) != 1 || reloadedStandalone["skills"].(float64) != 1 || opened["ok"] != true || openedStandalone["ok"] != true {
		t.Fatalf("unexpected Plugins results")
	}
	if NewAgentKit().Plugins != Plugins || NewAgentKit().PluginUI != PluginUI || NewAgentKit().PluginToggle != PluginToggle || NewAgentKit().PluginReload != PluginReload || NewAgentKit().PluginFiles != PluginFiles || NewAgentKit().PluginFolder != PluginFolder {
...
func TestSkillsNamespaceManagesRuntimeSkills(t *testing.T) {
	...
	dynamicStandalone, _ := SkillsDynamic.List(ctx)
	approvedStandalone, _ := SkillsDynamic.Approve(ctx, "draft_doc", "use safely")
	rejectedStandalone, _ := SkillsDynamic.Reject(ctx, "old_skill")
	...
	suggestionsStandalone, _ := SkillsSuggestions.Suggestions(ctx, "sess-1")

	if list["count"].(float64) != 1 || catalog["skills"] == nil || scan["status"] != "scanned" || scanStandalone["status"] != "scanned" || dynamic["skills"] == nil || dynamicStandalone["skills"] == nil || approved["status"] != "ok" || approvedStandalone["status"] != "ok" || rejected["name"] != "old_skill" || rejectedStandalone["name"] != "old_skill" || suggestions["suggestions"] == nil || suggestionsStandalone["suggestions"] == nil {
		t.Fatalf("unexpected Skills results")
	}
	if NewAgentKit().Skills != Skills || NewAgentKit().SkillsCatalog != SkillsCatalog || NewAgentKit().SkillsScan != SkillsScan || NewAgentKit().SkillsSuggestions != SkillsSuggestions || NewAgentKit().SkillsDynamic != SkillsDynamic {
