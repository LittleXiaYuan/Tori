...
describe("PackRuntimeRouteClientPage DLC route", () => {
  ...
  it("explains iframe-bundle DLC boundaries before rendering the sandbox host", async () => {
    ...
    expect(screen.getByText(/^当前入口/)).toBeInTheDocument();
    expect(screen.queryByText(/你现在打开的是/)).not.toBeInTheDocument();
    ...
    expect(screen.getByText(/回中心看状态，详情复查权限，再打开入口复验。/)).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /^权限与详情$/ })).toHaveAttribute("href", "/packs/detail?id=yunque.pack.dlc-demo");
    expect(screen.queryByRole("link", { name: /看权限与来源/ })).not.toBeInTheDocument();
    const handoffLink = screen.getByRole("link", { name: /^小羽打磨$/ });
    expect(handoffLink).toHaveAttribute("href", expect.stringContaining("/packs/studio?packId=yunque.pack.dlc-demo"));
    expect(screen.getAllByRole("link", { name: /回中心定位/ })).toHaveLength(1);
    expect(screen.queryByRole("link", { name: /回中心筛选/ })).not.toBeInTheDocument();
