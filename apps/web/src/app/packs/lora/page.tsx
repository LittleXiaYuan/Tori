...
import { createLoRAPackClient } from "@/lib/lora-pack-client";
...
const loraBoundaries: PackBoundaryItem[] = [
  { key: "force", label: "不强行训练", detail: "不会绕过样本量和间隔限制强行训练。" },
  { key: "lowq", label: "不收低质样本", detail: "不会自动把低质量样本加入训练。" },
  { key: "rollback", label: "不自动回滚", detail: "不会在没有确认时自动回滚。" },
  { key: "promise", label: "不保证提升", detail: "不会保证每次训练都提升模型表现。" },
];
...
export default function LoRAPackPage() {
  ...
  const onPreview = async () => {
    setBusy("preview");
    try {
      const res = await loraPack.preview();
  ...
  const onSaveConfig = async () => {
    ...
    try {
      ...
      await loraPack.updateConfig(patch);
  ...
  const onRollback = async () => {
    const confirmed = await confirmAction({
      title: "回滚到上一适配器？",
      body: "当前 LoRA 适配器将回滚到上一版本。请确认你已经保存需要保留的训练记录和评估结果。",
      confirmLabel: "执行回滚",
      cancelLabel: "取消",
      tone: "warning",
    });
  ...
  return (
    <div className="page-root space-y-6 animate-fade-in-up">
      <PageHeader
        ...
        iconColor="#a78bfa"
        title="LoRA 训练 Pack"
        description="由 Pack Runtime 启停的 LoRA/LAA 训练、回滚与进化能力包"
        onRefresh={load}
        actions={
      ...
      <PackAbout
        ...
        description="把云雀积累的任务样本转成 LoRA/LAA 训练闭环：预检数据、手动触发训练、观察 A/B 与滚动成功率，表现不稳时手动回滚。"
        boundaries={loraBoundaries}
      />
