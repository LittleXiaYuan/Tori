...
const userFacingSteps: PackStep[] = [
  { key: "save", label: "保存流程轨迹", detail: "把重复网页流程写成带参数的步骤，例如打开报表、选择月份、准备导出。" },
  { key: "plan", label: "生成回放计划", detail: "用参数替换模板，先得到 dry-run 计划和阻塞原因，不直接操作浏览器。" },
  { key: "evidence", label: "导出交接证据", detail: "把轨迹、参数、Browser Intent/ActionTracer 交接计划打包给后续执行器审核。" },
];

const boundaryItems: PackBoundaryItem[] = [
  { key: "click", label: "不操作网页", detail: "不会点击网页、输入表单或下载文件。" },
  { key: "intent", label: "不消费 Intent", detail: "不会消费 Browser Intent 会话或写浏览器状态。" },
  { key: "network", label: "不访问网络", detail: "不会访问外部目标站点或执行真实网络动作。" },
  { key: "done", label: "不当已完成", detail: "不会把 plan-only 轨迹当成已完成的自动化任务。" },
];

const workflowLoopItems: PackStep[] = [
  { key: "save", label: "保存轨迹", detail: "把网页流程、参数和断言保存成可复用记录，先形成可检查的自动化意图。" },
  { key: "chat", label: "带回 Chat", detail: "让云雀解释回放计划、补齐缺失步骤，或拆成需要人工确认的任务。" },
  { key: "evidence", label: "看证据位置", detail: "dry-run 结果和 executor handoff 是验证材料，不等于已经执行成功。" },
  { key: "extend", label: "继续补能力", detail: "如果还缺点击、截图或站点适配，把真实问题交给小羽继续改包。" },
];
  ...
  return (
    <div className="page-root space-y-6 animate-fade-in-up">
      ...
      <PackAbout
        ...
        description="它用于把重复网页流程整理成可复用轨迹，并预演“如果交给未来执行器会怎么做”。当前阶段主要帮助你保存步骤、替换参数、生成 dry-run 回放计划和证据包；它还不是能直接接管浏览器的完整 RPA。"
        boundaries={boundaryItems}
      />

      <Card variant="default">
        <Card.Header className="flex-row flex-wrap items-center justify-between gap-2">
          <PackSectionTitle icon={<Route size={15} />} tone="accent">怎么用</PackSectionTitle>
        ...
        <Card.Content className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2">
            <Chip size="sm" color={status?.executor_ready ? "success" : "warning"}>
              {status?.executor_ready ? "Executor ready" : "Pack shell"}
            </Chip>
            <span className="font-mono text-xs text-muted">{status?.pack_id || "yunque.pack.rpa-replay"}</span>
          </div>
      ...
      <Card variant="default">
        <Card.Header className="flex-row flex-wrap items-start justify-between gap-3">
          <div className="flex flex-col gap-1">
            <PackSectionTitle icon={<Workflow size={15} />} tone="accent">从回放计划到可验证自动化</PackSectionTitle>
            <span className="text-xs leading-5 text-muted">RPA Replay 的价值不是假装已经接管浏览器，而是把重复流程沉淀成可审计计划，再交给 Chat、任务中心和小羽逐步补成真能力。</span>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[360px_1fr]">
        <Card className="section-card overflow-hidden">
          ...
          <div className="max-h-[520px] divide-y overflow-auto border-border">
            {traces.length === 0 ? (
              <div className="p-6 text-center text-sm text-muted">还没有轨迹。可以先保存右侧示例。</div>
            ) : traces.map((trace) => (
              <button key={trace.slug} onClick={() => setSlug(trace.slug)} className="block w-full px-4 py-3 text-left hover:bg-white/5">
                ...
                <div className="mt-1 truncate text-xs text-muted">{trace.slug}</div>
              </button>
            ))}
          </div>
        </Card>
        ...
        <div className="space-y-4">
          <Card className="section-card p-4">
            ...
            <div className="mb-2 text-xs font-medium text-foreground">轨迹定义 JSON</div>
          ...
          <Card className="section-card p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-muted">目标轨迹：{selectedTrace?.slug || slug}</div>
              </div>
            ...
            <div className="mb-2 text-xs font-medium text-foreground">回放参数 JSON</div>
            ...
            {result && (
              <Card className="mt-3 p-3 bg-surface-secondary">
                ...
                <pre className="max-h-72 overflow-auto whitespace-pre-wrap text-xs text-muted">{JSON.stringify(result, null, 2)}</pre>
              </Card>
            )}
            {executorPlan && (
              <Card className="mt-3 p-3 bg-surface-secondary">
                ...
                <pre className="max-h-72 overflow-auto whitespace-pre-wrap text-xs text-muted">{JSON.stringify(executorPlan, null, 2)}</pre>
              </Card>
            )}
          </Card>
        </div>
