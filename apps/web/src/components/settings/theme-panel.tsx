...
import { Button, Chip, Tabs, Slider, Label, ToggleButton, ToggleButtonGroup, ColorPicker, ColorArea, ColorSlider, ColorSwatch, ColorField, Tooltip } from "@heroui/react";
import {
        ...
        <Tabs.Panel id="appearance" className="space-y-3 pt-4">
          ...
          <div className="section-card" style={{ padding: "var(--card-pad-sm)" }}>
            ...
            <div className="flex flex-wrap gap-2.5 items-center">
              {COLOR_THEMES.map(c => (
                <Tooltip key={c.id} delay={0}>
                  <Button isIconOnly aria-label={`颜色主题: ${c.name}`} onPress={() => upd({ colorTheme: c.id })}
