...
func TestDiffPackBundlesReportsAddRemoveChangeAndToggle(t *testing.T) {
	baseCompanion := PersonalCompanionPack()
	oldOnly := PackManifest{ID: "old-only", Version: "0.1.0", Type: "cogni"}
	from, err := NewPackBundle("before", []PackManifest{baseCompanion, oldOnly}, []string{PackPersonalCompanion, "old-only"})
	if err != nil {
	...
	if len(diff.ChangedPacks) != 1 || diff.ChangedPacks[0].ID != PackPersonalCompanion || diff.ChangedPacks[0].Reason != "version changed" {
	...
	if !containsString(diff.DisabledPacks, PackPersonalCompanion) || !containsString(diff.DisabledPacks, "old-only") {
		t.Fatalf("disabled packs mismatch: %#v", diff.DisabledPacks)
	}
}

func TestRenderPackBundleDiffMarkdown(t *testing.T) {
	from, err := NewPackBundle("before", []PackManifest{PersonalCompanionPack()}, []string{PackPersonalCompanion})
	if err != nil {
		t.Fatalf("from bundle: %v", err)
	}
	to, err := NewPackBundle("after", []PackManifest{PersonalCompanionPack(), YunqueWorkPack()}, []string{PackPersonalCompanion, PackYunqueWork})
	if err != nil {
		t.Fatalf("to bundle: %v", err)
	}
	diff, err := DiffPackBundles(from, to)
...
func TestDiffPackBundlesRejectsInvalidBundle(t *testing.T) {
	valid, err := NewPackBundle("valid", []PackManifest{PersonalCompanionPack()}, []string{PackPersonalCompanion})
	if err != nil {
