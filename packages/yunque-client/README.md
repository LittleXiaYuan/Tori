...
Reflection Experience, Mission Parse, Scheduler, Cron System, Triggers, Memory Kernel, Knowledge Graph, Knowledge Base, LoRA, Workflow, Connector, Notify, Cost, Providers, Cognis, Trace, Heartbeat, Events, Reverie, Tori, Upload, Speech, Setup, and Plugin API Runtime clients without importing the
...
import { normalizeCognitionSdkPackageManifest } from "yunque-client/cognisdk-package";
...
import { createSBOMDriftCIClient } from "yunque-client/sbom-drift-ci";
...
const backupInfo = await backup.info();
console.log(backupInfo.file_count);
...
const sbomDrift = createSBOMDriftClient({
  baseUrl: "http://localhost:9090",
  apiKey: "<your-api-key>",
});
...
| `src/cognisdk-package.ts` | Local-only Cognition SDK package manifest helpers for frontends, plugin pages, release gates, and automation scripts that need package entrypoints without importing the full generated client |
...
| `src/sbom-drift.ts` | Lightweight SBOM Drift Pack client for dependency snapshots, drift diff reports, and JSON evidence export without generated SDK imports |
| `src/sbom-drift-ci.ts` | Narrow SBOM Drift CI workflow handoff helper without snapshot/diff or evidence APIs |
...
The lightweight Packs SDK reads `/v1/packs/installed`, `/v1/packs/enabled`, `/v1/packs/catalog`, `/v1/packs/release-catalog`, and `/v1/packs/backend-modules` so the frontend shell can synchronize menus, frontend route bindings, backend route method/path bindings, UI assets, SDK entrypoints, GitHub Release `.yqpack` cards, and catalog `source_reports` from the backend pack registry instead of hard-coding every capability in the main app.
...
const releaseCatalog = await packs.releaseCatalog([
  "https://github.com/LittleXiaYuan/Tori/releases/tag/pack%2Fmicro-agent%2Fv0.1.0",
]);
await packs.install({
  packageUrl: releaseCatalog.entries[0]!.package_url,
  sha256: releaseCatalog.entries[0]!.sha256,
  source: releaseCatalog.entries[0]!.release_url,
});
...
### Cognition SDK Package Manifest Helpers

The lightweight `cognisdk-package` slice is also local-only. It lets external frontends, plugin pages, release gates, and automation scripts consume `sdk/manifest/cognisdk-package.json`, show the Go/CLI/test/doc entrypoints, and verify that a bundle really targets the rollback-friendly `pkg/cognisdk` package surface.
...
} from "yunque-client/cognisdk-package";

const manifest = normalizeCognitionSdkPackageManifest(await response.json());
const summary = summarizeCognitionSdkPackageManifest(manifest);
const entrypoints = listCognitionSdkPackageEntrypoints(manifest);
console.log(summary.capabilityCount, entrypoints.filter((entry) => entry.kind === "cli"));
```
