...
test("CognitiveCanaryClient reads status and scenarios with bearer token", async () => {
  ...
  const client = createCognitiveCanaryClient({
    ...
    fetch: async (url, init) => {
      ...
      if (String(url).endsWith("/status")) return jsonResponse({ pack_id: "yunque.pack.cognitive-canary", stage: "pack-shell-before-shadow-traffic", shadow_plan_ready: true, shadow_traffic_ready: false, judge_plan_ready: true, judge_pipeline_ready: false, response_collector_plan_ready: true, response_collector_store_ready: true, response_collector_writeback_ready: true, writes_response_collector_store: true, response_collector_pipeline_plan_ready: true, response_collector_pipeline_ready: false, consumes_response_collector_store: true, response_collector_ready: false, response_collector_store: { artifact: "response-collector-store.json", record_count: 0, response_collector_pipeline_plan_ready: true, consumes_response_collector_store: true }, metrics_plan_ready: true, prometheus_ready: false, quality_sli_ready: true, auto_rollback_plan_ready: true, auto_rollback_ready: false, scenario_count: 1, report_count: 0, policy: {}, capabilities: ["canary.response_collector.plan", "canary.response_collector.writeback", "canary.response_collector.pipeline.plan"] });
  ...
  assertEqual(status.response_collector_pipeline_plan_ready, true);
  assertEqual(status.consumes_response_collector_store, true);
...
test("CognitiveCanaryClient saves scenarios, evaluates canaries, plans shadow traffic, writes response collector store, plans pipeline handoff, and reads report detail", async () => {
  const calls: { url: string; init?: RequestInit }[] = [];
  const client = createCognitiveCanaryClient({
    baseUrl: "http://localhost:9090",
    apiKey: "key-123",
    fetch: async (url, init) => {
      ...
      if (String(url).endsWith("/response-collector/pipeline/plan")) return jsonResponse({ plan: { pack_id: "yunque.pack.cognitive-canary", generated_at: "now", status: "response_collector_pipeline_handoff_plan", report_id: "canary-1", record_count: 1, response_collector_pipeline_plan_ready: true, response_collector_pipeline_ready: false, consumes_response_collector_store: true, response_collector_store_ready: true, response_collector_writeback_ready: true, writes_response_collector_store: true, response_collector_ready: false, shadow_traffic_ready: false, judge_pipeline_ready: false, prometheus_ready: false, auto_rollback_ready: false, writes_files: false, records: [], response_collector_store: { pack_id: "yunque.pack.cognitive-canary", store: "pack-local-json", store_ready: true, record_count: 1, artifact: "response-collector-store.json", response_collector_store_ready: true, response_collector_writeback_ready: true, writes_response_collector_store: true, response_collector_pipeline_plan_ready: true, consumes_response_collector_store: true, response_collector_pipeline_ready: false, response_collector_ready: false, shadow_traffic_ready: false, judge_pipeline_ready: false, prometheus_ready: false, auto_rollback_ready: false }, response_collector_pipeline_plan: { target: "runtime.cognitive_canary.response_collector_pipeline", source_store: "response-collector-store.json", report_id: "canary-1", record_ids: ["canary-collector-1"], pair_ids: ["runtime-quality-check-abc"], artifacts: ["response-collector/runtime-quality-check-abc.json"], artifact: "response-collector-handoff-plan.json", artifact_sha256: "b".repeat(64), artifact_bytes: 192, dedup_key: "c".repeat(64), consumes_response_collector_store: true, writes_live_response_artifacts: false, writes_judge_batches: false, writes_prometheus_metrics: false, writes_rollback_state: false, response_collector_pipeline_ready: false, response_collector_ready: false, shadow_traffic_ready: false, judge_pipeline_ready: false, prometheus_ready: false, auto_rollback_ready: false, approval_required: false, actions: [], blocked_by: ["live-response-collector-not-wired"] }, artifacts: ["response-collector-pipeline-plan.json", "response-collector-handoff-plan.json"], actions: [], labels: [] } });
  ...
  assertEqual(pipeline.plan.response_collector_pipeline_plan_ready, true);
  assertEqual(pipeline.plan.consumes_response_collector_store, true);
  assertEqual(pipeline.plan.response_collector_pipeline_ready, false);
  assertEqual(pipeline.plan.response_collector_pipeline_plan.artifact_sha256.length, 64);
  ...
  assertEqual(calls[4]?.url, "http://localhost:9090/v1/cognitive-canary/response-collector/pipeline/plan");
  assertEqual(calls[4]?.init?.method, "POST");
  assertEqual(calls[4]?.init?.body, JSON.stringify({ report_id: "canary-1", requested_by: "unit" }));
  assertEqual(calls[5]?.url, "http://localhost:9090/v1/cognitive-canary/reports/canary-1");
...
test("CognitiveCanaryClient lists reports and exports evidence packs", async () => {
  ...
  assertDeepEqual(evidence.files, ["canary-report.json", "shadow-plan.json", "response-collector-plan.json", "response-collector-store.json", "response-collector-record.json", "response-collector-pipeline-plan.json", "response-collector-handoff-plan.json", "judge-plan.json", "metrics-plan.json", "rollback-plan.json"]);
  ...
  assertEqual(evidence.response_collector_pipeline_plan_ready, true);
