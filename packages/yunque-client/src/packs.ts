...
export type PackInstallRequest = { manifestPath?: string; manifestUrl?: string; packagePath?: string; packageUrl?: string; sha256?: string; source?: string; download?: boolean };
...
export type PackStudioAuditRequest = { workspacePath?: string; workspace_path?: string; goal?: string };
export type PackStudioWorkspaceChange = { path: string; kind: string; status: string; editable: boolean; needs_source?: boolean; old_sha256?: string; new_sha256?: string; reason?: string; [key: string]: unknown };
export type PackStudioAuditReport = { generated_at: string; workspace_path: string; workspace_id: string; original_sha256: string; current_sha256: string; manifest: PackManifest; change_count: number; editable_change_count: number; guarded_change_count: number; allowed: boolean; risk_level: "low" | "medium" | "high" | string; changes: PackStudioWorkspaceChange[]; warnings?: string[]; next_steps: string[]; [key: string]: unknown };
export type PackStudioRepackRequest = { workspacePath?: string; workspace_path?: string; outPath?: string; out_path?: string; goal?: string };
export type PackStudioRepackReport = { generated_at: string; workspace_path: string; package_path: string; sha256: string; size_bytes: number; manifest: PackManifest; inspect: YqpackInspectReport; warnings?: string[]; next_steps: string[]; [key: string]: unknown };
...
export class PacksClient {
  ...
  studioAudit(request: PackStudioAuditRequest): Promise<PackStudioAuditReport> { return this.json<PackStudioAuditReport>("POST", "/v1/packs/studio/audit", { workspace_path: request.workspace_path ?? request.workspacePath, goal: request.goal }); }
  studioRepack(request: PackStudioRepackRequest): Promise<PackStudioRepackReport> { return this.json<PackStudioRepackReport>("POST", "/v1/packs/studio/repack", { workspace_path: request.workspace_path ?? request.workspacePath, out_path: request.out_path ?? request.outPath, goal: request.goal }); }
  install(request: PackInstallRequest): Promise<PackMutationResponse> { return this.json<PackMutationResponse>("POST", "/v1/packs/install", { manifest_path: request.manifestPath, manifest_url: request.manifestUrl, package_path: request.packagePath, package_url: request.packageUrl, sha256: request.sha256, source: request.source, download: request.download }); }
