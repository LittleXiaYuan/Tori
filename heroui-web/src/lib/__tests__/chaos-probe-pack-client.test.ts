...
describe("chaos-probe-pack-client", () => {
  ...
  it("plans scheduler write-back, writes pack-local degrade state, and exports JSON evidence packs by report id", async () => {
    const spy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(
        new Response(
    ...
    expect(spy.mock.calls[2]?.[0]).toBe("/v1/chaos-probe/evidence/chaos-1");
  });
});
