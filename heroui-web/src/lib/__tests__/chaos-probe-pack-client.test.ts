...
describe("chaos-probe-pack-client", () => {
  it("reads Chaos Probe status, definitions, and reports through pack-owned routes", async () => {
    const spy = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ pack_id: "yunque.pack.chaos-probe", stage: "pack-shell-before-scheduler", safe_probe_ready: true, scheduler_plan_ready: true, scheduler_ready: false, metrics_plan_ready: true, prometheus_ready: false, degrade_writeback_plan_ready: true, degrade_engine_ready: false, alert_writeback_plan_ready: true, alert_writeback_ready: false, probe_count: 1, report_count: 1, policy: {}, capabilities: [] }), { status: 200 }))
  ...
  it("plans scheduler write-back and exports JSON evidence packs by report id", async () => {
    const spy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response(JSON.stringify({ plan: { pack_id: "yunque.pack.chaos-probe", status: "schedule_plan", interval: "5m", scheduler_plan_ready: true, scheduler_ready: false, metrics: [], actions: [] } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ pack_id: "yunque.pack.chaos-probe", exported_at: "now", format: "json-chaos-probe-evidence", files: ["chaos-report.json", "scheduler-plan.json"], report: { id: "chaos-1", results: [] } }), { status: 200 }));
    ...
    expect(JSON.parse(String((spy.mock.calls[0]?.[1] as RequestInit).body))).toEqual({ report_id: "chaos-1", interval: "5m" });
    expect(spy.mock.calls[1]?.[0]).toBe("/v1/chaos-probe/evidence/chaos-1");
  });
});
