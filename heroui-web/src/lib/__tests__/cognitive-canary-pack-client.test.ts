...
describe("cognitive-canary-pack-client", () => {
  it("reads Cognitive Canary status, scenarios, and reports through pack-owned routes", async () => {
    const spy = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ pack_id: "yunque.pack.cognitive-canary", stage: "pack-shell-before-shadow-traffic", shadow_plan_ready: true, shadow_traffic_ready: false, judge_plan_ready: true, judge_pipeline_ready: false, response_collector_plan_ready: true, response_collector_store_ready: true, response_collector_writeback_ready: true, writes_response_collector_store: true, response_collector_ready: false, response_collector_store: { record_count: 0, artifact: "response-collector-store.json", response_collector_store_ready: true, writes_response_collector_store: true }, metrics_plan_ready: true, prometheus_ready: false, quality_sli_ready: true, auto_rollback_plan_ready: true, auto_rollback_ready: false, scenario_count: 1, report_count: 1, policy: {}, capabilities: ["canary.response_collector.plan", "canary.response_collector.writeback"] }), { status: 200 }))
  ...
  it("saves scenarios, evaluates canaries, plans shadow rollout, and writes response collector store", async () => {
    ...
    expect(spy.mock.calls[4]?.[0]).toBe("/v1/cognitive-canary/reports/canary-1");
  });

  it("exports JSON evidence packs by report id", async () => {
    const spy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response(JSON.stringify({ pack_id: "yunque.pack.cognitive-canary", exported_at: "now", format: "json-cognitive-canary-evidence", files: ["canary-report.json", "shadow-plan.json", "response-collector-plan.json", "response-collector-store.json", "response-collector-record.json", "judge-plan.json", "metrics-plan.json", "rollback-plan.json"], report: { id: "canary-1", results: [] }, shadow_plan: { shadow_plan_ready: true, shadow_traffic_ready: false, response_collector_plan_ready: true, response_collector_ready: false, response_collectors: [], response_collector_summary: { writes_files: false, hash_algorithm: "sha256" } }, response_collector_store: { artifact: "response-collector-store.json", record_count: 1 }, response_collector_records: [{ record_id: "canary-collector-1", writes_response_collector_store: true, response_collector_ready: false }] }), { status: 200 }));
