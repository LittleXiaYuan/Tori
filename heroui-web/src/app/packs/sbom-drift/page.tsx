...
import { AlertTriangle, Download, GitCompare, PackageSearch, RefreshCw, ShieldCheck } from "lucide-react";
import PageHeader from "@/components/page-header";
import { showToast } from "@/components/toast-provider";
import { formatErrorMessage } from "@/lib/error-utils";
import { createSBOMDriftPackClient, type SBOMDriftDiff, type SBOMDriftSnapshotSummary, type SBOMDriftStatus } from "@/lib/sbom-drift-pack-client";
...
export default function SBOMDriftPackPage() {
  ...
  const [busy, setBusy] = useState<"snapshot" | "diff" | "evidence" | null>(null);
  ...
  return (
    <div className="page-root space-y-6 animate-fade-in-up">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="text-sm" style={{ color: "var(--yunque-text-muted)" }}>
              当前切片先完成 Go/npm 依赖快照、漂移 diff、风险分级和证据包导出。CycloneDX 生成、govulncheck 阻断和 CI 对比将在后续切片接入。
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <Card className="section-card p-4"><div className="kpi-label">CVE 阻断</div><div className="kpi-value text-lg">{status?.vulnerability_ready ? "ready" : "pending"}</div></Card>
