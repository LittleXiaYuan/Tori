...
import { Activity, AlertTriangle, Download, Radar, RefreshCw, ShieldAlert } from "lucide-react";
import PageHeader from "@/components/page-header";
import { showToast } from "@/components/toast-provider";
import { formatErrorMessage } from "@/lib/error-utils";
import { createSkillAnomalyPackClient, type SkillAnomalyProfileSummary, type SkillAnomalyResult, type SkillAnomalyStatus } from "@/lib/skill-anomaly-pack-client";
...
export default function SkillAnomalyPackPage() {
  ...
  const [busy, setBusy] = useState<"observe" | "detect" | "evidence" | null>(null);
          ...
          <div>
            <div className="mb-1 flex items-center gap-2">
              <Chip size="sm" style={{ background: status?.audit_hook_ready ? "rgba(34,197,94,0.12)" : "rgba(250,204,21,0.12)", color: status?.audit_hook_ready ? "#22c55e" : "#facc15" }}>
                {status?.audit_hook_ready ? "Audit hook ready" : "Pack shell"}
              </Chip>
            ...
            <div className="text-sm" style={{ color: "var(--yunque-text-muted)" }}>
              当前切片先把 skill 行为基线、滑动窗口异常评分、NeedsApproval 计划和证据包放进可选 Pack。Merkle Chain 直连 hook、Trust Score 惩罚和审批队列写回会在后续切片接入。
            </div>
          </div>
