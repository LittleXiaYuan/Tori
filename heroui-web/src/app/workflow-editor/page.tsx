...
function WorkflowEditorContent() {
  ...
  if (!workflow) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-default-400">
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] overflow-hidden">
      {/* Left: Palette */}
      <div className="w-48 border-r border-divider bg-content1 overflow-y-auto shrink-0 flex flex-col">
        <div className="p-3 border-b border-divider">
          <div className="text-xs font-semibold uppercase text-default-500 tracking-wider">节点库</div>
        </div>
        <div className="p-2 space-y-1 flex-1">
          {NODE_TYPES.map(nt => (
            <div key={nt.type} draggable
              onDragStart={(e) => { e.dataTransfer.setData("text/plain", nt.type); e.dataTransfer.effectAllowed = "move"; }}
              className="flex items-center gap-2 p-2 rounded-lg text-xs cursor-grab active:cursor-grabbing hover:bg-default-100 transition-colors select-none">
        ...
        <div className="p-3 border-t border-divider">
          <div className="text-[10px] text-default-400 leading-relaxed">拖拽到画布 · Alt+拖拽平移 · 滚轮缩放 · 拖出端口连线</div>
        </div>
      </div>

      {/* Center: Canvas */}
      <div className="flex-1 relative bg-default-50"
        onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; }}>
        {/* Toolbar */}
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
          <Button size="sm" variant="ghost" onPress={() => router.push("/missions")} className="bg-content1 border border-divider shadow-sm">
            <ArrowLeft size={14} />
            返回
          </Button>
          <div className="px-3 py-1.5 rounded-lg bg-content1 border border-divider text-xs shadow-sm">
            <span className="font-medium">{workflow.name}</span>
            <span className="text-default-400 ml-2">v{workflow.version} · {workflow.nodes.length}节点 · {workflow.edges.length}连线</span>
          </div>
        </div>
        <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
          <Button isIconOnly aria-label="放大" size="sm" variant="ghost" onPress={() => setZoom(z => Math.min(3, z * 1.2))} className="bg-content1 border border-divider shadow-sm"><ZoomIn size={14} /></Button>
          <span className="text-xs text-default-400 w-10 text-center">{Math.round(zoom * 100)}%</span>
          <Button isIconOnly aria-label="缩小" size="sm" variant="ghost" onPress={() => setZoom(z => Math.max(0.2, z * 0.8))} className="bg-content1 border border-divider shadow-sm"><ZoomOut size={14} /></Button>
          <Button size="sm" variant="ghost" onPress={() => { setPan({ x: 40, y: 40 }); setZoom(1); }} className="bg-content1 border border-divider shadow-sm"><RotateCcw size={14} /> 重置</Button>
          <Button size="sm" isPending={saving} onPress={saveWorkflow}
        ...
        <svg ref={svgRef} className="w-full h-full"
          ...
          <defs>
            <pattern id="grid" width={20 * zoom} height={20 * zoom} patternUnits="userSpaceOnUse" x={pan.x % (20 * zoom)} y={pan.y % (20 * zoom)}>
              <circle cx={1} cy={1} r={0.5} fill="currentColor" className="text-default-200" />
            </pattern>
            <marker id="ah" markerWidth={10} markerHeight={7} refX={10} refY={3.5} orient="auto">
              <polygon points="0 0, 10 3.5, 0 7" fill="currentColor" className="text-default-300" />
            </marker>
          </defs>
                ...
                <g key={edge.id} className="cursor-pointer" onClick={() => removeEdge(edge.id)}>
                  ...
                  {edge.condition && (
                    <text x={(from.position.x + to.position.x + NODE_W) / 2} y={(from.position.y + NODE_H + to.position.y) / 2 - 4}
                      fontSize={10} fill="currentColor" className="text-default-400" textAnchor="middle">{edge.condition}</text>
                  )}
                </g>
              );
            })}
              ...
              return (
                <g key={node.id} transform={`translate(${node.position.x}, ${node.position.y})`}>
                  ...
                  <rect width={NODE_W} height={NODE_H} rx={10} ry={10}
                    fill="var(--heroui-content1, #fff)" stroke={isSel ? c : "var(--heroui-divider, #e5e5e5)"}
                  ...
                  <text x={36} y={NODE_H / 2 - 6} fontSize={12} fontWeight={500} fill="currentColor" className="text-foreground pointer-events-none">
                    {node.name.length > 12 ? node.name.slice(0, 12) + "…" : node.name}
                  </text>
                  <text x={36} y={NODE_H / 2 + 10} fontSize={10} fill="currentColor" className="text-default-400 pointer-events-none">{typeLabel(node.type)}</text>
      ...
      {selNode && (
        <div className="w-72 border-l border-divider bg-content1 overflow-y-auto shrink-0">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="border-t border-divider pt-4">
              <label className="text-xs text-default-500 mb-1 block">位置</label>
            ...
            <div><label className="text-xs text-default-500 mb-1 block">节点 ID</label>
              <div className="text-xs font-mono text-default-300 truncate">{selNode.id}</div></div>
            <div className="border-t border-divider pt-4">
              <label className="text-xs text-default-500 mb-2 block">连线</label>
              {workflow.edges.filter(e => e.from_node === selNode.id || e.to_node === selNode.id).length === 0
                ? <div className="text-xs text-default-300">无连线</div>
                : <div className="space-y-1">{workflow.edges.filter(e => e.from_node === selNode.id || e.to_node === selNode.id).map(edge => {
                    ...
                    return (<div key={edge.id} className="flex items-center justify-between p-1.5 rounded bg-default-50 text-xs">
                      <span>{edge.from_node === selNode.id ? "→" : "←"} {other?.name || "?"}</span>
                      <Button isIconOnly aria-label="关闭" size="sm" variant="ghost" onPress={() => removeEdge(edge.id)} className="text-default-300 hover:text-danger">
                        <X size={12} />
                      </Button></div>);
                  })}</div>}
            </div>
