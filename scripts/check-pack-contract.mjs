...
for (const token of [
  ...
  "retention-prune-execute.json",
  "pack-local retention prune",
...
for (const token of ["Pack Runtime completion audit", "RegisterBackendPack", "frontendSync()", "capabilityBindings", "PruneArtifacts", "TypeScript packs SDK", "backup-pack 示例包"]) {
...
const gatewaySource = readText("internal/controlplane/gateway/handlers_packs.go")
  ...
  + "\n"
  + readText("internal/config/config.go")
  + "\n"
  + readText("internal/config/config_test.go")
  + "\n"
...
if (!cogniKernelSource.includes("type API interface") || !cogniKernelSource.includes("type Router struct") || !cogniKernelSource.includes("NewRouter")) {
  fail("Cogni Kernel pack handler must own API interface and router before delegating to Gateway adapter");
}
if (!cogniGatewayBridge.includes("ServeCogniKernel") || !cogniGatewayBridge.includes("g.handleCognis(w, r)")) {
  fail("Cogni Kernel Gateway adapter must delegate through ServeCogniKernel while extraction continues");
}
...
for (const token of ["TestSBOMDrift", "StatusNotFound", "StatusMethodNotAllowed", "/v1/sbom-drift/diff", "/v1/sbom-drift/baseline/artifact-source/plan", "/v1/sbom-drift/ci-gate/baseline/writeback", "/v1/sbom-drift/ci-gate/workflow/writeback/plan"]) {
...
for (const token of ["BaselineArtifactSourcePlan", "baseline-artifact-source-plan.json", "baseline-fetch-handoff-plan.json", "artifact_source_plan_ready", "baseline_fetch_plan_ready", "fetches_artifact_baseline", "writes_baseline_snapshot"]) {
  if (!sbomDriftSource.includes(token)) fail(`SBOM Drift handler missing artifact baseline source plan token: ${token}`);
}
...
for (const token of ["TestMemoryTimeTravel", "StatusMethodNotAllowed", "/v1/memory-time-travel/diff", "/v1/memory-time-travel/rollback/approved-plan", "/v1/memory-time-travel/retention/prune/execute", "/v1/memory-time-travel/kv-history/native-plan", "/v1/memory-time-travel/kv-history/migration-preview", "/v1/memory-time-travel/kv-history/dual-read/parity", "/v1/memory-time-travel/kv-history/cutover/plan", "/v1/memory-time-travel/kv-history/cutover/readiness", "/v1/memory-time-travel/audit/links/preview", "/v1/memory-time-travel/audit/links/writeback-plan", "/v1/memory-time-travel/audit/links/writeback/store", "/v1/memory-time-travel/audit/links/writeback/executor/plan"]) {
  if (!memoryTimeTravelGateTest.includes(token)) fail(`Memory Time Travel gateway gate test missing token: ${token}`);
}
for (const token of ["createMemoryTimeTravelClient", "MemoryTimeTravelClientError", "/v1/memory-time-travel/status", "/v1/memory-time-travel/rollback/approved-plan", "/v1/memory-time-travel/retention/plan", "/v1/memory-time-travel/retention/prune-plan", "/v1/memory-time-travel/retention/prune/execute", "/v1/memory-time-travel/kv-history/native-plan", "/v1/memory-time-travel/kv-history/migration-preview", "/v1/memory-time-travel/kv-history/dual-read/parity", "/v1/memory-time-travel/kv-history/cutover/plan", "/v1/memory-time-travel/kv-history/cutover/readiness", "/v1/memory-time-travel/audit/links", "/v1/memory-time-travel/audit/links/preview", "/v1/memory-time-travel/audit/links/writeback-plan", "/v1/memory-time-travel/audit/links/writeback/store", "/v1/memory-time-travel/audit/links/writeback/executor/plan", "/v1/memory-time-travel/audit/verify", "/v1/memory-time-travel/evidence/", "approvedRollbackPlan", "approved_rollback_plan", "rollback_writeback_plan", "approval_request_plan", "retentionPlan", "retentionPrunePlan", "retentionPruneExecute", "pack_local_prune_ready", "writes_pack_local_snapshot_store", "nativeKVHistoryPlan", "nativeKVHistoryMigrationPreview", "kvHistoryDualReadParity", "kvHistoryCutoverPlan", "kvHistoryCutoverReadiness", "auditLinksPreview", "native_kv_history_plan", "kv_history_migration_plan", "kv_history_index_plan", "kv_history_migration_preview", "kv_history_dual_read_parity", "native_kv_history_preview_ready", "kv_history_cutover_plan", "kv_history_cutover_readiness", "kv_history_dual_read_plan", "kv_history_dual_write_plan", "kv_history_cutover_plan_ready", "kv_history_cutover_readiness_ready", "cutover_readiness_check_ready", "dual_read_plan_ready", "dual_read_parity_check_ready", "dual_read_parity_ready", "parity_passed", "dual_write_plan_ready", "cutover_ready", "auditLinks", "auditLinksPreview", "auditLinksWritebackPlan", "auditLinksWritebackStore", "auditLinksWritebackExecutorPlan", "auditVerify", "kv_audit_link_writeback_plan", "kv_audit_link_writeback_actions", "kv_audit_link_writeback_store", "kv_audit_link_writeback_records", "kv_audit_link_writeback_plan_ready", "kv_audit_link_writeback_store_ready", "kv_audit_link_writeback_ready", "writes_audit_link_writeback_store", "backfills_audit_seq", "backfills_audit_hash", "consumes_audit_link_preview", "kv_audit_link_writeback_executor_plan", "kv_audit_link_writeback_executor_plan_ready", "executor_input_contract_ready", "audit_proof_link_executor_ready", "consumes_audit_link_writeback_store", "audit_append_plan_ready", "writes_audit_chain"]) {
...
for (const token of ["TestWASMPlugin", "StatusNotFound", "StatusMethodNotAllowed", "/v1/wasm-plugin/execute", "/v1/wasm-plugin/remote-install/plan", "/v1/wasm-plugin/remote-install/approval/plan", "/v1/wasm-plugin/remote-install/approval/decision/plan", "/v1/wasm-plugin/remote-install/approval/writeback/plan", "/v1/wasm-plugin/remote-install/approval/queue/writeback", "/v1/wasm-plugin/remote-install/installer/continuation/plan", "/v1/wasm-plugin/remote-install/installer/download/writeback", "/v1/wasm-plugin/remote-install/signature-verification/writeback"]) {
  if (!wasmPluginGateTest.includes(token)) fail(`WASM Plugin gateway gate test missing token: ${token}`);
}
for (const token of ["createWASMPluginClient", "WASMPluginClientError", "/v1/wasm-plugin/status", "/v1/wasm-plugin/remote-install/plan", "/v1/wasm-plugin/remote-install/approval/plan", "/v1/wasm-plugin/remote-install/approval/decision/plan", "/v1/wasm-plugin/remote-install/approval/writeback/plan", "/v1/wasm-plugin/remote-install/approval/queue/writeback", "/v1/wasm-plugin/remote-install/installer/continuation/plan", "/v1/wasm-plugin/remote-install/installer/download/writeback", "/v1/wasm-plugin/remote-install/signature-verification/writeback", "/v1/wasm-plugin/evidence/", "remoteInstallPlan", "remoteInstallApprovalPlan", "remoteInstallApprovalDecisionPlan", "remoteInstallApprovalWritebackPlan", "remoteInstallApprovalQueueWriteback", "remoteInstallInstallerContinuationPlan", "remoteInstallInstallerDownloadWriteback", "remoteInstallSignatureVerificationWriteback", "WASMPluginRemoteInstallPlan", "WASMPluginRemoteInstallApprovalPlan", "WASMPluginRemoteInstallApprovalDecisionPlan", "WASMPluginRemoteInstallApprovalWritebackPlan", "WASMPluginRemoteInstallInstallerContinuationPlan", "WASMPluginRemoteInstallInstallerDownloadWriteback", "WASMPluginRemoteInstallSignatureVerificationWriteback", "WASMPluginInstallerContinuationPlan", "WASMPluginInstallerDownloadRecord", "WASMPluginSignatureVerificationPlan", "WASMPluginApprovalQueueEntryPlan", "WASMPluginApprovalDecisionPlan", "WASMPluginApprovalWritebackPlan", "remote_install_plan", "signature_verification", "approval_queue_entry", "approval_gate_plan", "approval_decision_plan", "approval_writeback_plan", "installer_download_record"]) {
