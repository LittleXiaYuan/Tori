...
| 6   | control-plane  | 🟡  | 路由所有权已覆盖 governance/approvals/inbox/tools/bots/plugins/metrics/system/tenants/providers 等；observability / approvals / tenants / inbox 已原生，其余仍通过 gateway bridge |
...
- knowledge：无，已全原生。
- control-plane：plugins / models / tools / bots / providers 及部分 governance 面仍通过 `HandleControlPlanePack` 桥接，后续继续按低风险 surface 小切片迁移。
