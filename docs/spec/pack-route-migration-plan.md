...
| 6   | control-plane  | 🟡  | 路由所有权已覆盖 governance/approvals/inbox/tools/bots/plugins/metrics/system/tenants/models/providers 等；observability / audit / approvals / tenants / inbox / bots / tools / models / usage/quota 已原生，其余仍通过 gateway bridge |
...
- knowledge：无，已全原生。
- control-plane：plugins / providers 及 trust/iterate/review/skillgrow governance 面仍通过 `HandleControlPlanePack` 桥接，后续继续按低风险 surface 小切片迁移。
...
> 2026-06-18 增量：control-plane tenants 集合路由已原生（GET list / POST create），gateway 仅提供 `TenantManager()` 窄 accessor。
...
> 2026-06-18 增量：control-plane `/v1/models` 已原生；模型目录状态抽为 `internal/controlplane/models`，pack 通过 `ModelManager()` / `ProviderModels()` / `DeleteProviderModel()` 窄 accessor 合并显式模型与 provider 派生模型，保留隐藏派生模型与删除 provider 的旧语义。

> 2026-06-18 增量：control-plane `/v1/usage` / `/v1/quota` 已原生；UsageTracker 继续留在 gateway 生命周期内，pack 通过 `UsageSnapshot()` / `SetUsageQuota()` 窄 accessor 读当前租户用量与写配额。

> 2026-06-18 增量：control-plane audit 四条路由已原生（`/v1/audit/tail`、`/v1/audit/verify`、`/v1/audit/stats`、`/api/audit/trail`），gateway 仅暴露 `AuditChain()` / `AuditTrail()` 只读对象 accessor。
