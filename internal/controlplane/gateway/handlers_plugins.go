...
//  from handlers_presets.go 
// handlePresets lists all presets (GET) or switches the active preset (POST).
...
// handlePresetFeatures updates the feature toggles for a preset (PUT).
...
// handleCustomPreset creates (POST) or deletes (DELETE) a custom preset.
