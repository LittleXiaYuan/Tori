...
type Config struct {
	...
	HandoffAsyncEnabled      bool   // kill switch for background sub-agent handoff execution (default true)
	HandoffConcurrencyXiaoyu int    // max concurrent async sub-agent handoffs per session in 小羽模式 (default 1)
	HandoffConcurrencyAPI    int    // max concurrent async sub-agent handoffs per session in API模式 (default 4)
...
func Load() Config {
	...
	return Config{
		...
		HandoffAsyncEnabled:      getenv("HANDOFF_ASYNC_ENABLED", "true") == "true",
		HandoffConcurrencyXiaoyu: getenvInt("HANDOFF_CONCURRENCY_XIAOYU", 1),
		HandoffConcurrencyAPI:    getenvInt("HANDOFF_CONCURRENCY_API", 4),
